 ...
 */
float **WAVtoMFCCs(int index,int *pLen)
{
	WavFileHeaderStruct *fileHeader ;//wav结构体定义文件头
	float* WAVData,**f,**mfcc;
	int i,n,frameLen,frameN,start,end;
	int sampleRate;
	fileHeader = openWAVFile(index);//获取文件头
	if (!fileHeader)
	{
		fprintf(stderr, "1111error：%s:%d\n", __FILE__, __LINE__);
		return NULL;
	}
	WAVData = readAllWAVData(fileHeader, &n);//读取wav音频数据
	sampleRate = fileHeader->FormatChunk.SampleRate;//wav的采样率
	...
	f = framing(WAVData,n,frameLen,frameLen/3,&frameN);//分帧
	free(WAVData);
